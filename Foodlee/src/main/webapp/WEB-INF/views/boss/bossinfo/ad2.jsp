...
// 	    	var form = $('#BannerLink'+a)[0];
// 	        var formData = new FormData(form);

// 	    	var data = new FormData();
// 	    	jQuery.each(jQuery('#banner_file1'+a)[0].files, function(i, file) {
// 	    	    data.append('file-'+i, file);
// 	    	});
			
			var imgData = new FormData();
			imgData.append("ups", $("input[name=upfiles]")[index].files[0]);
			
	        var badType = $("#banner_type"+index).val();
	        imgData.append( "baType", badType);
	        
	        var price = $("#banner_price"+index).val();
	        imgData.append(	"adPrice", price);
	        
	        var badate = $("#banner_date"+index).val();
	        imgData.append(	"adStartDate", badate);
	        ...
	        $.ajax({
	            ...
	            processData: false,
			    type: "post",
			    url: '${pageContext.request.contextPath}/banner_apply.fdl',
			    data: imgData,
			    success: function(data, textStatus) {
			    		console.log("요청사항");
			    }
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="javascript:void(0)" data-toggle="dropdown"
                                ...
                                <img src="assets/images/users/profile-pic.jpg" alt="user" class="rounded-circle"
                                    width="40">
                                <span class="ml-2 d-none d-lg-inline-block"><span style="color: #fff">안녕하세요 ${LoginName}님</span> 
				...
				<div class="card-body">
					<div class="d-flex d-lg-flex d-md-block align-items-center">
						<div>
							...
							<div class="d-inline-flex align-items-center">
								<h2 class="text-dark mb-1 font-weight-medium">${baType}</h2>
...
<%-- 						    <form id="banner_form1" class="mt-4" action="${pageContext.request.contextPath}/banner_apply.fdl" enctype="multipart/form-data" method="post"> --%>
						    <form id="BannerLink0" class="mt-4" enctype="multipart/form-data">
								<div class="form-header">
									<input id="banner_type0" type="hidden" name="baType" value="파워링크"><h2>파워링크</h2>
<!-- 									<p>Fill out this form to start your free trial!</p> -->
								</div>
						        <div class="form-group">
								<h1 class="text-center"><input id="banner_price0" type="hidden" name="adPrice" value="55000">55,000<small><i class="fas fa-won-sign"></i></small></h1>
								...
								<div class="form-group">
									<label>적용기간</label>
						            <input id="banner_date0" type="date" class="form-control" name="adStartDate" required="required">
						            <!-- 오늘 날짜 default로 !! -->
						        </div>
						        <div class="form-group"> 
						        	<label>파워링크 이미지 등록<br><span style="color: red; font-size: 10px;" >(1200 x 110 pixel) 사이즈가 안맞을시 신청취소가 되실수 있습니다</span></label>
						        	<input id="banner_file0" type="file" name="upfiles" placeholder="첨부파일명들" multiple="multiple" size='64'>
						        </div>
								<div class="form-group">
									<button type="button" onclick="selectButton(0)" id="banner_add0" class="btn btn-primary btn-block btn-lg">등록</button>
								</div>	
						    </form>
						    </div>
						    <div class="signup-form pl-4 mb-5">
<%-- 						    <form id="banner_form2" class="mt-4" action="${pageContext.request.contextPath}/banner_apply.fdl" enctype="multipart/form-data" method="post"> --%>
						    <form id="BannerLink1" class="mt-4" enctype="multipart/form-data">
								<div class="form-header">
									<input id="banner_type1" type="hidden" name="baType" value="배너광고"><h2>배너광고</h2>
<!-- 									<p>Fill out this form to start your free trial!</p> -->
								</div>
						        <div class="form-group">
								<h1 class="text-center"><input id="banner_price1" type="hidden" name="adPrice" value="45000">45,000<small><i class="fas fa-won-sign"></i></small></h1>
						        </div>
						        <div class="form-group">
								...
								<div class="form-group">
									<button type="button" onclick="selectButton(1)" id="banner_add1" class="btn btn-primary btn-block btn-lg">등록</button>
								</div>	
						    </form>
						    </div>
						</div>
