...
@Service
public class FoodtruckFileManageSVCImpl implements IFoodtruckFileSVC {
...
//			} catch (IllegalStateException | IOException e) {
	...
	public Map<String, Object> writeUploadedMultipleFiles(List<MultipartFile> files, String realPath, String login) {
		StringBuffer sb = new StringBuffer();
		long totalByte = 0L;
		for (int i = 0; i < files.size(); i++) {
			MultipartFile uf = files.get(i);
			String fp = this.writeUploadedFile(uf, realPath, login);
			if (fp != null) {
				sb.append(fp + (i != files.size() - 1 ? MULTI_SEP : ""));
